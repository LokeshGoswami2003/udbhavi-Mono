...
export const resumeDataSchema = {
  basics: {
    ...
    portfolio: "string",
    summary: "string",
    links: [{ label: "GitHub | YouTube | Live | Snapshots | Portfolio | Certificate | Link", url: "string", type: "repo | video | live | snapshot | certificate | portfolio | other" }],
  },
  ...
  projects: [
    {
      name: "string",
      description: "string",
      url: "string (legacy single URL — keep for backward compatibility, may equal links[0].url)",
      links: [
        {
          label: "Visible label such as GitHub | YouTube | Live | Snapshots | Repo | Portfolio | Certificate | Link",
          url: "string",
          type: "repo | video | live | snapshot | certificate | portfolio | other",
        },
      ],
      bullets: ["string"],
      technologies: ["string"],
...
export function buildInitialResumePrompt({ project, resume, template }) {
  ...
  return {
    system: [
      ...
      "Projects can have multiple links. Always populate projects[].links[] with every URL that belongs to a project (GitHub, YouTube, Live, Snapshots, etc.).",
      "Do NOT collapse a project's GitHub + YouTube + Live URLs into one entry. Keep them as separate entries in projects[].links[].",
      "Use the visible label from the source (GitHub, YouTube, Live, Snapshots, etc.) as projects[].links[].label.",
      "Set projects[].links[].type to one of: repo, video, live, snapshot, certificate, portfolio, other.",
      "Keep projects[].url for backward compatibility but the renderer will prefer projects[].links[].",
      "If sourceLinks contains project, demo, or repository URLs, assign them to the most likely project.links[] only when the label, context, or project name supports the match.",
